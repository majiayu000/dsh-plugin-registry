...
(function () {
  ...
  var en = {
    ...
    '自动发现流程': 'Auto-discovery flow', '声明 bundle': 'Declare bundle', '添加 Topic': 'Add topic', '自动验证': 'Automatic verification', '进入列表': 'Get listed',
    '每 8 小时': 'Every 8 hours', '无需提交表单': 'No submission form', '自动发现最低契约': 'Minimum auto-discovery contract', '自动发现要求根目录 package.json 声明 dsh.bundle。': 'Auto-discovery requires dsh.bundle in the root package.json.',
    ...
    '检查自动发现资格': 'Check auto-discovery eligibility', '检查公开元数据、Topic 与根目录 package.json；不会运行代码，也不代表安全审计。': 'Checks public metadata, the topic, and the root package.json. It does not run code and is not a security audit.',
