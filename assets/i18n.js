...
(function () {
  ...
  var en = {
    ...
    '社区精选与 GitHub 自动发现的插件都在这里。可安装条目会校验 dsh.bundle；有 verifiedCommit 时，安装命令会钉到该 commit。': 'Curated picks and auto-discovered plugins in one place. Installable entries are checked for dsh.bundle; when a verifiedCommit exists, the install command pins that commit.',
    ...
    '全部插件': 'All plugins', '插件与候选仓库': 'Plugins and candidate repositories', '插件': 'Plugins', '官方': 'Official', '安装': 'Install', '已复制 ✓': 'Copied ✓',
    ...
    '自动发现要求根目录或子包 package.json 声明 dsh.bundle，且 patch 是顶层 YAML 数组（允许空数组）。': 'Auto-discovery requires dsh.bundle in the root or a subpackage package.json, and the patch must be a top-level YAML array (empty arrays are allowed).',
    ...
    '声明有效的 dsh.bundle': 'Declares a valid dsh.bundle', 'Patch 文件是顶层 YAML 数组': 'Patch file is a top-level YAML array', '仓库未归档且不是 Fork': 'Repository is unarchived and not a fork',
    ...
    '仓库必须同时满足以下条件，才能进入可安装插件列表。缺少有效 dsh.bundle、但具有额外 DSH 关联信号的候选仓库不会出现在默认列表里，只在筛选「缺少有效 dsh.bundle」时展示，且不提供安装命令。': 'A repository must meet every requirement below to enter the installable plugin list. Candidates without a valid dsh.bundle stay out of the default list; they appear only when filtering for a missing dsh.bundle, and they do not get an install command.',
    ...
    '必须是安全的相对路径；引用文件必须存在，且必须是顶层 YAML 数组（允许空数组）。精选条目在能够检查时使用同一套契约。': 'must be a safe relative path; the referenced file must exist and be a top-level YAML array (empty arrays are allowed). Curated entries use the same contract when they can be checked.',
    ...
    '仓库已被发现，但不出现在默认插件列表中，也不提供安装命令；补充有效声明后会自动转为可安装状态。': 'The repository has been discovered, but it is not in the default plugin list and has no install command. It becomes installable automatically after adding a valid declaration.',
