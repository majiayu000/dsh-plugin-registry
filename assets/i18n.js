...
(function () {
  ...
  var en = {
    ...
    '路由、检索、评测、通知 —— 社区插件和 GitHub 候选仓库都在这里。先看数据来源和 Manifest 状态，再到仓库确认依赖与配置。': 'Routing, retrieval, evaluation, and notifications — browse community plugins and GitHub candidates together. Check the data source and Manifest status, then confirm dependencies and setup in the repository.',
    ...
    '收录插件': 'Plugins', '可浏览仓库': 'Browsable repositories', '本周下载': 'Weekly installs', '认证作者': 'Verified authors', '兼容 CLI 版本': 'Compatible CLI',
    ...
    '本周精选': 'Featured this week', '全部插件': 'All plugins', '插件与候选仓库': 'Plugins and candidate repositories', '每周一更新': 'Updated every Monday',
    ...
    , '加载更多': 'Load more',
    '收录 · 本站规则': 'Listing · Registry policy', '怎样进入公开列表。': 'How plugins enter the public registry.', '怎样进入可安装列表。': 'How plugins enter the installable registry.',
    ...
    '自动收录条件': 'Automatic listing requirements', '仓库必须同时满足以下条件，缺少任何一项都不会进入公开插件列表。': 'A repository must meet every requirement below before it can enter the public plugin list.', '仓库必须同时满足以下条件，才能进入可安装插件列表；缺少有效 dsh.bundle 的候选仓库仍会公开展示并标注状态。': 'A repository must meet every requirement below to enter the installable plugin list. Candidates without a valid dsh.bundle remain visible with a clear status.',
    '仓库必须同时满足以下条件，才能进入可安装插件列表；缺少有效 dsh.bundle、但具有额外 DSH 关联信号的候选仓库仍会公开展示并标注状态。': 'A repository must meet every requirement below to enter the installable plugin list. Candidates without a valid dsh.bundle remain visible only when they provide an additional DSH relevance signal.',
    ...
    '候选相关性明确': 'Candidate relevance established', '缺少有效 bundle 时，仓库名称、简介或额外 Topic 必须还能表明它与 DSH 有关；仅有发现 Topic 的仓库不会展示。': 'Without a valid bundle, the repository name, description, or an additional topic must still establish a DSH connection. The discovery topic alone is not enough for display.',
    ...
    '根目录 package.json 的 dsh.bundle.patch 必须是安全的相对路径，且引用文件真实存在。': 'The root package.json must provide dsh.bundle.patch as a safe relative path whose referenced file exists.',
    'Manifest 与 Patch 已确认': 'Manifest and patch confirmed', '自动发现，根目录 bundle 格式有效，且引用的 Patch 文件存在；可进入可安装列表。': 'Automatically discovered with a valid root bundle and an existing referenced patch file; eligible for the installable list.',
    ...
    '缺少有效 dsh.bundle': 'Valid dsh.bundle missing', '仓库已被发现并显示在浏览列表中，但不提供安装命令；补充有效声明后会自动转为可安装状态。': 'The repository is visible in the directory, but no install command is provided. It becomes installable automatically after adding a valid declaration.',
