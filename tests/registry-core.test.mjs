...
test('only a dsh.bundle with a safe relative patch passes manifest validation', () => {
  ...
  assert.equal(hasBundleManifest('{"dsh":{"bundle":{"patch":"./%2e%2e/secret.yml"}}}'), false)
  assert.equal(hasBundleManifest('{"dsh":{"bundle":{"patch":"./foo/%2e%2e/bar.yml"}}}'), false)
  assert.equal(hasBundleManifest('{"dsh":{"bundle":{"patch":"./%2E%2E/secret.yml"}}}'), false)
  assert.equal(hasBundleManifest('{"dsh":{"bundle":{"patch":"./..%2fsecret.yml"}}}'), false)
