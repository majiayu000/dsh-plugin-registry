...
test('install actions open an accessible shared guide instead of silently copying', async () => {
  ...
  assert.match(shared, /data-install-profile/)
  assert.match(shared, /任意目录；需能运行 dsh 和 pnpm/)
  assert.match(shared, /data-install-verify-command/)
  assert.match(shared, /list --depth 0/)
  assert.match(shared, /启动 dsh web 验证插件功能/)
